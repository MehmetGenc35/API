...
//button type submit yapmak forma girilen bilgilerin valule larını action ile istenilen yere gönderir
//eğer form action ile bir yer belirtilmemişse defaultta sayfayı yeniler bilgiler tekrar kendine döner

// novalidate==> formun browser tarafındaki validasyonunu iptal eder
...
const addUser = async() => { 
    ...
    const user = {//backend deki field isimleriyle aynı olması gerekiyor
        /*firstName: firstName,
        lastName: lastName,
        email: email  key ve value aynı ise js bunu bizim yazdığımız gibi olmasına izin verir
        firstName,lastName,email
    };
    await createUser(user);
    fillUserList();//backend kısmına yeni userla beraber listeyi tekrar yüklesin dedik
 }
*/
